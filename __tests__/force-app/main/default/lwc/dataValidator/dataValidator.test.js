...
describe('DataValidator.validatePhone', () => {
  ...
  it('rejects 9 digits as too short', () => {
    ...
    expect(res).toEqual({ valid: false, error: 'Phone must be 10-15 digits' })
  })
...
describe('DataValidator.validateRequired', () => {
  it('rejects null/undefined/empty string with default message', () => {
    const cmp = createInstance()
    expect(cmp.validateRequired(null)).toEqual({ valid: false, error: 'Field is required' })
    expect(cmp.validateRequired(undefined)).toEqual({ valid: false, error: 'Field is required' })
    expect(cmp.validateRequired('')).toEqual({ valid: false, error: 'Field is required' })
  })
  ...
  it('treats 0 and false as valid values', () => {
    ...
    expect(cmp.validateRequired(false)).toEqual({ valid: true, error: null })
  })
})
...
describe('DataValidator.validateLength', () => {
  ...
  it('rejects string shorter than min', () => {
    ...
    expect(res).toEqual({ valid: false, error: 'Length must be between 2 and 5' })
  })
  ...
  it('handles arrays with length correctly', () => {
    const cmp = createInstance()
    expect(cmp.validateLength([1, 2, 3], 2, 3)).toEqual({ valid: true, error: null })
    expect(cmp.validateLength([], 1, 3)).toEqual({ valid: false, error: 'Length must be between 1 and 3' })
  })

  it('returns length error for non-length values like numbers', () => {
    const cmp = createInstance()
    const res = cmp.validateLength(123, 1, 4)
    expect(res).toEqual({ valid: false, error: 'Length must be between 1 and 4' })
  })
})
...
describe('DataValidator.validateNumericRange', () => {
  ...
  it('accepts lower and upper bounds inclusively', () => {
    const cmp = createInstance()
    expect(cmp.validateNumericRange(1, 1, 10)).toEqual({ valid: true, error: null })
    expect(cmp.validateNumericRange(10, 1, 10)).toEqual({ valid: true, error: null })
  })

  it('rejects below min', () => {
    const cmp = createInstance()
    const res = cmp.validateNumericRange(0, 1, 10)
    expect(res).toEqual({ valid: false, error: 'Value must be between 1 and 10' })
  })
...
describe('DataValidator.validatePostalCode', () => {
  ...
  it('validates US ZIP codes', () => {
    const cmp = createInstance()
    expect(cmp.validatePostalCode('12345', 'US')).toEqual({ valid: true, error: null })
    expect(cmp.validatePostalCode('12345-6789', 'US')).toEqual({ valid: true, error: null })
    expect(cmp.validatePostalCode('1234', 'US')).toEqual({ valid: false, error: 'Invalid US postal code format' })
  })

  it('validates Canadian postal codes (case-insensitive and optional space)', () => {
    const cmp = createInstance()
    expect(cmp.validatePostalCode('K1A 0B1', 'CA')).toEqual({ valid: true, error: null })
    expect(cmp.validatePostalCode('k1a0b1', 'CA')).toEqual({ valid: true, error: null })
    expect(cmp.validatePostalCode('123 456', 'CA')).toEqual({ valid: false, error: 'Invalid CA postal code format' })
  })

  it('validates UK postal codes per implemented pattern', () => {
    const cmp = createInstance()
    expect(cmp.validatePostalCode('EC1 1BB', 'UK')).toEqual({ valid: true, error: null })
    expect(cmp.validatePostalCode('SW1A 1AA', 'UK')).toEqual({ valid: false, error: 'Invalid UK postal code format' })
  })
