...
describe('DataFormatter LWC Utility Methods', () => {
  ...
  describe('formatPhoneNumber', () => {
    ...
    it('formats 11-digit numbers with leading 1 as +1 (US)', () => {
      el = makeEl()
      expect(el.formatPhoneNumber('+1 (234) 567-8901')).toBe('+1 (234) 567-8901')
      expect(el.formatPhoneNumber('12345678901')).toBe('+1 (234) 567-8901')
    })
  ...
  describe('formatCurrency', () => {
    ...
    it('formats numeric string amount', () => {
      el = makeEl()
      const amount = '1000'
      const expected = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(Number(amount))
      expect(el.formatCurrency(amount)).toBe(expected)
    })
  ...
  describe('formatDate', () => {
    ...
    it('formats date in medium, long and full formats', () => {
      el = makeEl()
      const ds = '2024-07-15T12:00:00Z'
      expect(el.formatDate(ds, 'medium')).toBe(expectedFormatted(ds, 'medium'))
      expect(el.formatDate(ds, 'long')).toBe(expectedFormatted(ds, 'long'))
      expect(el.formatDate(ds, 'full')).toBe(expectedFormatted(ds, 'full'))
    })
  ...
  describe('formatAddress', () => {
    ...
    it('formats full address including postal code', () => {
      el = makeEl()
      const address = { street: '123 Main St', city: 'Springfield', state: 'IL', postalCode: '62704', country: 'USA' }
    ...
    it('formats address without postal code but with trailing space before newline', () => {
      el = makeEl()
      const address = { street: '123 Main St', city: 'Springfield', state: 'IL', country: 'USA' }
      const expected = '123 Main St\nSpringfield, IL \nUSA'
      expect(el.formatAddress(address)).toBe(expected)
    })
    ...
    it('omits state when city is missing and places country on new line', () => {
      el = makeEl()
      const address1 = { street: '123 Main St', state: 'CA', country: 'USA' }
  ...
  describe('capitalizeWords', () => {
    ...
    it('capitalizes first letter of each word', () => {
      ...
      expect(el.capitalizeWords('multiple words here')).toBe('Multiple Words Here')
    })
  ...
  describe('sanitizeText', () => {
    ...
    it('escapes special HTML characters including slash', () => {
      ...
      const expected = '&lt;div class=&quot;x&quot;&gt;It&#x27;s &#x2F; ok&lt;&#x2F;div&gt;'
